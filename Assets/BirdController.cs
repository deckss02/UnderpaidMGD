...
public class BirdController : MonoBehaviour
{
    ...
    public BossTim bosstim;

    private Collider2D BirdCollider; // Reference to the rat's collider
    ...
    void Start()
    {
        ...
        bosstim = FindObjectOfType<BossTim>();
        currentHealth = maxHealth; // Initialize current health
        BirdCollider = GetComponent<Collider2D>(); // Get the Collider2D component attached to the rat
    ...
    private void Die()
    {
        if (!isKilled)
        {
            ...
            bosstim.KillBird(); // Notify the boss
    ...
    private IEnumerator EnableColliderAfterDelay(float delay)
    {
        ...
        BirdCollider.enabled = true; // Re-enable the collider after the delay
